...
wget -O data.csv https://raw.githubusercontent.com/yinghaoz1/tmdb-movie-dataset-analysis/master/tmdb-movies.csv 

# 1. Sort movies by release date in descending order and save to a new file
csvsort -r -c 16 data.csv > Project_03_1.csv
...
csvtool -t ',' col 1,14 data.csv | sed 's/, /,/g' | awk -F',' '{split($2, genres, "|"); for(i in genres) print $1 "," genres[i]}'  > Project_03_7_1.csv
csvsql --query "select genres, count(1) total from Project_03_7_1 group by genres" Project_03_7_1.csv > Project_03_7_2.csv
